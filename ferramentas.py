...
def informacoes_dataframe(pergunta: str, df: pd.DataFrame) -> str:
    """
    Gera um relatório com informações gerais sobre o dataframe: dimensões,
    colunas, tipos de dados, nulos, strings 'nan' e duplicados.
    """
    shape = df.shape
    columns = df.dtypes
    nulos = df.isnull().sum()
...
def resumo_estatistico(pergunta: str, df: pd.DataFrame) -> str:
    """
    Gera um relatório com estatísticas descritivas completas do dataframe
...
def gerar_grafico(pergunta: str, df: pd.DataFrame) -> str:
    """
    Gera e exibe (via st.pyplot) um gráfico a partir de uma instrução em
    linguagem natural, usando matplotlib e seaborn.
    """
    ...
    # Limpa o código gerado (remove marcações de bloco de código markdown)
...
def criar_ferramentas(df: pd.DataFrame):
    """
    Cria e retorna a lista de ferramentas (Tools) do agente, já vinculadas
    ao dataframe `df` fornecido.
    """

    ferramenta_informacoes_dataframe = Tool(
        name="Informações Dataframe",
        func=lambda pergunta: informacoes_dataframe(pergunta, df),
        description=(
            "Utilize esta ferramenta sempre que o usuário solicitar informações gerais sobre o "
            "dataframe, incluindo número de colunas e linhas, nomes das colunas e seus tipos de "
            "dados, contagem de dados nulos e duplicados para dar um panorama geral sobre o arquivo."
        ),
        return_direct=True,
    )
    ...
    ferramenta_resumo_estatistico = Tool(
        ...
        func=lambda pergunta: resumo_estatistico(pergunta, df),
        description=(
            "Utilize esta ferramenta sempre que o usuário solicitar um resumo estatístico completo "
            "e descritivo da base de dados, incluindo várias estatísticas (média, desvio padrão, "
            "mínimo, máximo etc.) e/ou múltiplas colunas numéricas. Não utilize esta ferramenta para "
            "calcular uma única métrica como 'qual é a média de X' ou 'qual a correlação das "
            "variáveis'. Para isso, use a ferramenta de Códigos Python."
        ),
        return_direct=True,
    )
    ...
    ferramenta_gerar_grafico = Tool(
        name="Gerar Gráfico",
        func=lambda pergunta: gerar_grafico(pergunta, df),
        description=(
            "Utilize esta ferramenta sempre que o usuário solicitar um gráfico a partir de um "
            "DataFrame pandas (`df`) com base em uma instrução do usuário. A instrução pode conter "
            "pedidos como: 'Crie um gráfico da média de tempo de entrega por clima', 'Plote a "
            "distribuição do tempo de entrega' ou 'Plote a relação entre a classificação dos agentes "
            "e o tempo de entrega'. Palavras-chave comuns que indicam o uso desta ferramenta incluem: "
            "'crie um gráfico', 'plote', 'visualize', 'faça um gráfico de', 'mostre a distribuição', "
    ...
    ferramenta_codigos_python = Tool(
        ...
        description=(
            "Utilize esta ferramenta sempre que o usuário solicitar cálculos, consultas ou "
            "transformações específicas usando Python diretamente sobre o DataFrame `df`. Exemplos "
            "de uso incluem: 'Qual é a média da coluna X?', 'Quais são os valores únicos da coluna "
            "Y?', 'Qual a correlação entre A e B?'. Evite utilizar esta ferramenta para solicitações "
            "mais amplas ou descritivas, como informações gerais sobre o dataframe, resumos "
            "estatísticos completos ou geração de gráficos — nesses casos, use as ferramentas "
            "apropriadas."
        ),
    )
